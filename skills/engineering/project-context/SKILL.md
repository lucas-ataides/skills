...
For deep, evolving memory, a project can adopt the **project brain** — a CLI-managed store (`scripts/brain.py`) the agent reads and writes without ever hand-editing, covered in step 6.
...
6. **Use the project brain for deep memory.** Run `scripts/brain.py list` to see the brain's pages, then `scripts/brain.py read <id>` for the ones the task touches, before working. Record a decision or shifted understanding with `scripts/brain.py truth <id> --text "…" --why "…"` or `scripts/brain.py timeline <id> --kind <type> --text "…"` — the CLI is the only writer, so a brain file is never hand-edited. The model, the CLI, and the page format live in [the project brain reference](references/project-brain.md). The step is done once the relevant pages are read for context, or `scripts/brain.py list` shows the brain is empty.
