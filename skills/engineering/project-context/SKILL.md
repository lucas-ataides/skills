...
2. **Create what is missing.** Run the helper with `init`. It writes an AGENTS.md only when neither AGENTS.md nor CLAUDE.md exists, writes a TODO.md when absent, and never overwrites an existing file. The step is done once an agent-instructions file and a TODO file both exist.
