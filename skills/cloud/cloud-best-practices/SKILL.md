...
5. **Gate the controls in CI.** Add policy-as-code and IaC scanning to the pipeline through [autoguardrails](../../engineering/autoguardrails/SKILL.md) and the gates in [secure-sdlc](../../engineering/secure-sdlc/SKILL.md), so a non-compliant change fails the build instead of reaching the account. Configure drift detection to flag any resource changed outside IaC. The step is done when a deliberately non-compliant fixture fails the gate and a drift event raises an alert.
